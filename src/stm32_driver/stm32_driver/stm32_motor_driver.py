...
class STM32MotorDriver(Node):
    ...
    def timer_callback(self):
        ...
        if current_time - self.last_sent_time > 0.4:
    ...
    def destroy_node(self):
        """节点被销毁时发送停止命令"""
        self.get_logger().info('🔴 正在发送停止命令给STM32...')
        try:
            # 直接发送停止命令（不通过 send_to_stm32 方法，避免复杂逻辑）
            stop_data = bytearray([0xAA, 0x55, 0x02, 0x04, 0x00, 0x00, 0x00, 0x00, 0x04])
            for i in range(5):  # 增加发送次数，确保停止命令被接收
