...
class SLAMAvoidance(Node):
    def __init__(self):
        super().__init__('slam_avoidance')
        ...
        # ========== 避障参数（针对 SLAM 优化）==========
        self.safe_distance = 0.8    # 安全距离（比原节点大）
        self.danger_distance = 0.6  # 危险距离（比原节点大）

        # 速度设置：为 SLAM 优化，但满足硬件要求（最低 0.45 m/s）
        self.min_speed = 0.45       # 最低速度（满足硬件要求 ≥0.45）
        self.max_speed = 0.55       # 最大速度（略高于最低要求，防止漂移）
        self.turn_speed = 0.6       # 转向速度（为 SLAM 优化，但不低于硬件要求）
        ...
        # 定时器：控制频率20Hz（降低频率，保证响应质量），建图1Hz（保证质量）
        self.create_timer(0.05, self.control_timer_callback)   # 20Hz
        self.create_timer(1.0, self.mapping_timer_callback)    # 1Hz

        self.get_logger().info('='*60)
        self.get_logger().info('🚗 SLAM 避障节点 (纯激光建图优化版)')
        self.get_logger().info('='*60)
        self.get_logger().info(f'   控制频率: 20Hz')
        self.get_logger().info(f'   最小速度: {self.min_speed}m/s')
        self.get_logger().info(f'   最大速度: {self.max_speed}m/s')
        self.get_logger().info(f'   转向速度: {self.turn_speed}rad/s')
        self.get_logger().info(f'   安全距离: {self.safe_distance}m')
        self.get_logger().info(f'   危险距离: {self.danger_distance}m')
        self.get_logger().info('='*60)
    ...
    def calculate_target_speed(self, scan):
        ...
        if front_dist < self.danger_distance:
            ...
            # 安全区域：速度随距离增加，但不低于硬件要求的最低速度
            if front_dist < self.safe_distance + 0.3:
                # 接近安全边界时，保持最低速度
                self.target_linear = self.min_speed
            else:
                # 距离足够时，缓慢增加到最大速度
